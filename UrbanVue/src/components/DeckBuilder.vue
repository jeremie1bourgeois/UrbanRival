...
import { DECK_SIZE, emptySlots, filterCatalogue, isDeckComplete, toDeck, type DeckSlots } from "../logic/deck";
...
const slots = ref<Record<Side, DeckSlots>>({ ally: emptySlots(), enemy: emptySlots() });

const results = computed(() => filterCatalogue(catalogue.value, query.value));
...
async function start() {
	...
	error.value = null;
	try {
		emit("start", await initGame(toDeck(slots.value.ally, slots.value.enemy)));
	} catch (err) {
		error.value = errorMessage(err);
	}
}
...
<template>
	<div class="mx-auto max-w-6xl p-6 space-y-6">
		<header class="flex items-center justify-between">
			...
			<button class="rounded bg-gray-700 px-3 py-2 text-sm hover:bg-gray-600" @click="startTemplate">Partie d'exemple (template)</button>
		</header>

		<p v-if="error" class="rounded border border-red-500 bg-red-900/40 p-3 text-red-200">{{ error }}</p>
		...
		<section class="grid grid-cols-2 gap-6">
			...
			>
				<div class="mb-3 flex items-center justify-between">
					<h2 class="font-bold">{{ which === "ally" ? "Allié" : "Ennemi" }}</h2>
					<button
						class="rounded px-2 py-1 text-xs"
						:class="side === which ? 'bg-yellow-500 text-black' : 'bg-gray-700'"
						@click="side = which"
					>
				...
				<ol class="space-y-1">
					...
					>
						<span v-if="slot"
							>{{ slot.card_name }} <span class="text-yellow-400">{{ "★".repeat(slot.nb_stars) }}</span></span
						>
		...
		<section class="space-y-3">
			<input
				...
				:disabled="loading"
				class="w-full rounded bg-gray-800 px-4 py-3 text-white outline-none ring-yellow-400 focus:ring-2"
			/>
			...
			<p v-else-if="query.trim() && !loading" class="text-gray-400">Aucune carte ne correspond.</p>
		</section>
