...
| D3 | Performance | Mesuré le 2026-09-18 : ~1 ms par round (deepcopy + `process_round`). Le solveur exact et la génération d'exemples (`docs/IA.md` § 5.3-5.4) demandent ~10 µs par round : état compact, sans deepcopy ni re-parsing des capacités, code compilé. **Port Rust** : le contrat (`src/core/engine/contract.py`) et le corpus combinatoire (`scripts/build_engine_corpus.py`, 113 501 rounds avec état suivant et issue du round) sont l'oracle de comparaison ; avant de figer le port, trancher les points ouverts `docs/REGLES.md` § 5 (R1 ordre entre camps, R2 per damage en défaite, R3 per life lost, R4 Protection contre « Cards ») pour ne pas porter des comportements arbitraires. |
